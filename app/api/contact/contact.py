...
def get_contacts_from_db(params):
    ...
    if 'order_type' in params.keys():
        ...
        if order_type == '최신순':
            sql = f"{sql} ORDER BY created_at DESC"  # 기존 쿼리 뒤에 ORDER BY created_at DESC추가
        elif order_type == '이름순':
    ...
    # DB 실행 결과 한줄 => (가공) => 클라이언트에게 전해줄 목록에 담기도록
    for row in query_result:
        # 클라이언트가 받아들이기 편리한 구조로 가공된 연락처를 담을 dict 만들기
        contact = {}        
        # contact의 내용을 채우자
        contact['id'] = row['id']
        ...
        # datetime 으로 오는 데이터 -> str로 가공해서 담아보자
        # datetime -> str : strftime 활용 (2022-01-08 02:01:00 양식)
