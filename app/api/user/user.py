...
def login_test(id, pw):
    ...
    # 지금의 None : 아이디는 있지만 비번이 틀린 것으로 간주
    if query_result == None :
        return {
            'code':400,
...
# 둘 다 통과 해야만 실제 INSERT INTO -> 결과를 200으로 내려주자 + 가입된 사용자 정보도 내려주자

def sign_up(params):
    
    # 이메일이 사용중인지? 검사
    # params['email']와 같은 이메일이 DB에 있는가? 조회 => SELECT를 날려보자
