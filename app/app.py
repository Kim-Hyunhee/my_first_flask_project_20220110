...
from .api.contact import get_all_contacts, add_contact_to_db
...
def create_app():
    ...
    def login_01():
        ...
        # 원하는 파라미터가 들어오지 않았다면? => 잘못 보냈다고 알려주는 코드 추가
        # dict의 key값의 데이터가 실제로 존재하는가?
        if 'login_id' not in params.keys() and 'pw' not in params.keys() :
            return {
                'code' : 400,
    ...
    def get_contacts_url():
        return get_all_contacts(request.args.to_dict())
